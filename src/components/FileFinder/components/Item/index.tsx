...
const Item: React.FC<{
  ...
  return (
    <Layout
      onNameClick={() => triggerChecked(file)}
      className={classNames(disabled ? 'opacity-30' : 'opacity-100')}
      icon={
        <Image
    ...
    >
      <Checkbox className={'flex-shrink-0'} checked={isChecked} disabled={disabled} />
      <Tooltip title={file.name} placement={'bottom'} mouseEnterDelay={0.3}>
        <div className={'pl-1 flex-1 text-ellipsis line-clamp-1 cursor-pointer'}>{file.name}</div>
      </Tooltip>
    </Layout>
  );
};
